...
class Library {
    ...
    async importCourse(courseDir, courseId = null) {
        ...
        if (!courseId) {
            ...
            const courseId = await this.createCourse(courseDir, coursePath + '/poster.jpg')
    ...
    importLesson(lessonDir, courseId) {
        console.log('create lesson: ' + lessonDir);
    ...
    convertLessonVideo(from, to) {
        const command = `ffmpeg -i "${from}" -map 0:v -c:v copy -map 0:a -c:a copy "${to}"`;
        return new Promise((resolve) => {
